...
noOfEditions int not null default 1,
...
claimDate date,
foreign key (cmId) references ContentManagers(cmId) on delete set null on update cascade,
constraint chk_amt_dates check (amount>=0 and (claimDate is null or claimDate>=paymentDate))
);
...
create table ItemEditedBy(
cmId int,
orderItemId int,
pubId int,
primary key (cmId,orderItemId,pubId),
foreign key (cmId) references Editors(cmId) on delete cascade on update cascade,
...
pubId int,
topicName varchar(50),
primary key (title, orderItemId, pubId, topicName),
foreign key (topicName) references Topics(topicName) on delete cascade on update cascade,
foreign key (title, orderItemId, pubId) references Chapters(title, orderItemId, pubId) on delete cascade on update cascade
);

create table ChapterWrittenBy(
title varchar(200),
orderItemId int,
pubId int,
cmId int,
...
distName varchar(200) not null,
distType varchar(50) not null,
balance float not null default 0,
primaryContact char(12) not null,
constraint chk_bal_disttype check (balance>=0 and distType in ('wholesale distributor', 'library', 'bookstore'))
);
...
receiptDate date,
distId int,
foreign key (distId) references Distributors(distId) on delete set null on update cascade,
constraint chk_amt_dates_bill check (billAmt>=0 and receiptDate>=generationDate)
);
...
primary key(orderId, orderItemId, pubId),
foreign key (orderId) references Orders(orderId) on delete cascade on update cascade,
foreign key (orderItemId, pubId) references OrderItems(orderItemId, pubId) on delete cascade on update cascade,
